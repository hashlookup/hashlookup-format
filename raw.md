...
This document describes the hashlookup output format used to express meta information of hash values seen in known databases.
The output description includes a common semantic. The hashlookup format is used for public or private services used in
...
Digital forensic is a critical field in information security and especially incident response. Providing details, intelligence
from known set of files is crucial to conduct digital investigations. hashlookup format provides a common output format for
...
Known File Filter (KFF) lists used in forensic software.
...
parsable by scripts or generated by software in stream mode. Each JSON object is expressed on a single
line to be processed by the client line-by-line. Examples of JSON output are below.

## Fields format

The hashlookup format is quite lax concerning the mandatory fields. The reason behind is the variety of
different sources and the granularity of the information available. The goal of the hashlookup format is
to share common fields to easily combine results.
...
A public hashlookup service [@?HASHLOOKUP-SERVICE] is provided by CIRCL and accessible as a ReST HTTP API. The software back-end which provides a hashlookup format
